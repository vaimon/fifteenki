...
using Board = std::array<unsigned short,16>;
...
void save(const std::string& filename) {
    ...
    if (!file.good()) {
        std::cerr << "Could not generate database file: databases/4-wd.dat " << filename
                  << std::endl;
        return;
    }
...
void WalkingDistance::load(const std::array<unsigned short,16>& goal, int w, int h) {
